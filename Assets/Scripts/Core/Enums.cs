...
public enum EAudioClip
{
    ...
    // 아이템 조작음 (#549). 새 항목을 중간에 끼우지 않고 뒤에 붙이는 이유는 값이
    ...
    // 다운 유예 구조 (#725). 뒤에 붙이는 이유는 위와 같다 — 정수로 직렬화돼 있다.
    ReviveLoop, // 웅 — 동료를 구조 채널링하는 동안 계속 (루프)

    // 유치장 경보등 (#311/#493). 뒤에 붙이는 이유는 위와 같다 — 정수로 직렬화돼 있다.
    JailAlarm, // 웨엥 — 경보등이 점멸하는 동안 계속 (루프)

    // NPC 근접 공격 (#817). 뒤에 붙이는 이유는 위와 같다 — 정수로 직렬화돼 있다.
    NpcAttackSwing, // 휙 — NPC가 주먹을 휘두른 순간 (명중 여부와 무관)
    NpcAttackHitRobot, // 깡 — NPC의 주먹이 로봇 경찰을 맞혔다 (플레이어는 전원 로봇이다)
}
