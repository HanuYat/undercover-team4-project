...
public abstract class ItemBase : ChanneledInteractionBehaviour
{
    ...
    ///
    /// 소모형의 첫 사례가 부활 키트(<see cref="ReviveKit"/>)라 경로를 여기 판다. 아이템 종류와
    /// 무관한 처리라 하위가 아니라 기반에 둔다.
    /// 구역 스캔(#490)은 소모형이 아니라 쿨타임형으로 확정돼(2026-08-23) 이 메서드를 쓰지 않는다 —
    /// 대신 <see cref="AreaScanner"/>가 <c>NetworkVariable&lt;double&gt;</c> 쿨다운 종료 시각을 쓴다
    /// (<see cref="JailSirenButton"/>과 같은 패턴).
    ///
