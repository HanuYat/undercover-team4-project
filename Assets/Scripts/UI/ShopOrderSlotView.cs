...
/// <summary>
/// 주문창 그리드의 한 칸 (#843). 진열대 슬롯 하나의 품목을 그리고, 주문 버튼을 창에 전달한다.
...
/// 칸마다 StringChanged를 걸지 않는 이유는 옛 진열대 표시와 같다: 항목이 여럿이라 로케일 변경
/// 한 곳에 걸고 통째로 다시 채우는 편이 해제 누락이 없다.
/// </summary>
public class ShopOrderSlotView : MonoBehaviour
{
    ...
    /// <summary>이 칸이 그리고 있는 진열대 슬롯. 빈 칸이면 null.</summary>
    public ShopStand Stand => m_stand;
    ...
    public void Bind(ShopStand stand)
    {
        m_stand = stand;

        ShopCatalog.Entry entry = stand != null ? stand.Entry : null;
        ...
        EStandStatus status = stand.Status;
        bool orderable = status == EStandStatus.Available;
        ...
        if (m_orderButtonText != null)
        {
            m_orderButtonText.color =
                status == EStandStatus.SoldOut ? m_soldOutColor
                : status == EStandStatus.Owned ? m_ownedColor
                : m_orderLabelColor;
        }
    ...
    private static string StatusKey(EStandStatus status) =>
        status == EStandStatus.SoldOut ? "Shop.Order.SoldOut"
        : status == EStandStatus.Owned ? "Shop.Order.Owned"
        : "Shop.Order.Button";
    ...
    {
        if (m_stand == null || m_owner == null)
            return;

        m_owner.RequestOrder(m_stand);
    }
