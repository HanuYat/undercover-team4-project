...
/// <summary>
/// 번역된 한 줄을 정해진 시간 동안 띄웠다가 서서히 지우는 HUD 표시의 공통 뼈대. (#493)
/// HUD 프리팹에 부착되며 App.UI를 통해 접근한다 — ChannelingGaugeUI와 동일 관례.
///
/// 파생 클래스는 <b>화면에서의 자리와 주인</b>만 다르다(<see cref="ToastView"/>는 상단 경보,
/// <see cref="SignalMessageView"/>는 중앙 무전 수신). 자리를 나눠 둔 덕분에 둘이 동시에 떠도
/// 서로를 덮지 않는다 — 예전 IMGUI처럼 좌표 상수를 서로 맞춰 둘 필요가 없다.
/// 파생 타입마다 App에 별도 필드가 있어야 한다(ManagerHandler는 정확한 타입으로 먼저 찾는다).
///
...
public abstract class TimedMessageView : CommonManagerBase
{
