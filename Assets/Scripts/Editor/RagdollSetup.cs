...
public static class RagdollSetup
{
    ...
    /// </param>
    /// <param name="stripHipsReplication">
    /// 골반에 남아 있는 <c>NetworkTransform</c> + <c>NetworkRigidbody</c>를 <b>걷어낼지</b> — <b>NPC 전용</b>.
    /// 전 뼈 자세 스트리밍으로 교체되면서 골반 복제가 필요 없어졌고, 남아 있으면 같은 서버 값의
    /// 다른 지연본이 자세와 싸운다. 플레이어는 배선이 다르고 아직 이관 전이라 끈다.
    /// </param>
    ...
    // ---- 골반 복제 해제 (전 뼈 자세 스트리밍) ----

    /// <summary>
    /// 골반에 붙어 있던 <c>NetworkTransform</c> + <c>NetworkRigidbody</c>를 <b>걷어낸다</b> — <b>멱등</b>.
    ///
    /// 예전에는 이것을 <b>붙이는</b> 함수였다(#572). 근거는 "뼈 11개 대신 골반 1개만 보내고
    /// 나머지는 각 피어의 로컬 물리가 관절로 만든다"였는데, <b>그 로컬 물리가 사라졌다</b> —
    /// 이제 원격은 <see cref="RagdollPoseStreamer"/>가 보내는 <b>전 뼈 자세</b>를 받아 그대로
    /// 입힌다. 골반을 따로 복제하면 같은 서버 값의 <b>다른 지연본</b>이 자세와 싸운다.
    ///
    /// <b>붙이는 것만큼 걷는 것도 자동화된다.</b> 붙는 자리가 <b>중첩 프리팩 안</b>이라
    /// 오버라이드로 저장되고, 리그를 다시 복제하면(<see cref="RagdollRigCloner"/>) 함께 걷힌다 —
    /// 손으로 지우면 다시 만들 때마다 되살아난다.
    ///
    /// ⚠ <b>순서가 있다.</b> <c>NetworkRigidbody</c>가 <c>NetworkTransform</c>을 <c>RequireComponent</c>하므로
    /// 그것을 먼저 떼지 않으면 Unity가 파괴를 거부한다.
    /// </summary>
