...
/// (Idle=0, Walk=1...). 100번대는 FSM 상태가 아닌 모션 전용 번호다 — 해제·기상처럼
...
public static class NpcAnimStates
{
    ...
    // 105·106은 제압 전환(그로기/구르기, #332)이 쓰던 번호다 — #502에서 걷혔다.
    // 제압은 래그돌로 쓰러지는 것으로 통일됐고, 그 구간에는 Animator가 아예 꺼져 있어
    // 클립이 재생될 자리가 없었다. 재사용하지 말 것: 갱신하지 않은 컨트롤러에 그 상태가
    // 남아 있으면 Any State 조건이 되살아나 엉뚱한 모션이 나온다.
