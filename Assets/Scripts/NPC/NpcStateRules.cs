...
public static class NpcStateRules
{
    ...
    /// <see cref="CanArrest"/>가 <see cref="NpcState.Escorted"/>를 제외한 채로 두는 것이 핵심이다:
    /// 그쪽을 열면 "새로 묶기" 경로가 통째로 열려 <b>탈취</b>(남의 끌기를 끊고 뺏기)가 딸려온다.
    /// 합류는 기존 끌기를 끊지 않고 참가자만 하나 늘린다 — 팀 결정은 "합류는 허용, 탈취는 차단"이다.
    ///
    /// 끌리는 중(Escorted)만이다. 놓아둔 체포(Captured)는 대상이 남의 소유로 서 있는 것이라
    /// 여기 넣으면 그게 곧 탈취가 된다.</summary>
