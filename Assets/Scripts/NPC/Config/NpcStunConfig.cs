...
public class NpcStunConfig : ScriptableObject
{
    ...
    [Tooltip("체력이 0이 되어 쓰러진 기절의 지속 시간(초) — 테이저와 분리된 값이다. 이 시간이 곧 밧줄로 끌 수 있는 창이라 검거 난이도의 손잡이다. 마지막 StandUpSeconds 구간은 일어나는 모션에 쓰이므로 실제로 누워 있는 시간은 그만큼 짧다")]
    [SerializeField] private float m_knockdownStunSeconds = 3.6f;
