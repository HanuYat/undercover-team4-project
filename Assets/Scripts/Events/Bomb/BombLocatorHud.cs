...
/// <summary>
/// 폭탄 위치 표시 — 화면 안이면 폭탄 위에 마커를, 밖이면 가장자리에 붙여 방향을 가리킨다. (#232, #399)
///
/// <b>추격 폭탄에서는 상시 표시가 맞다.</b> 고정 설치 시절엔 본부가 미니맵으로 찾아 무전으로 유도하는
/// 것이 협력 루프의 절반이라 이것이 임시 UI였다. 쫓아오는 지금은 반대다 — 어디서 오는지 안 보이면
/// 달아날 방향을 고를 수 없어 대응 자체가 성립하지 않는다.
///
/// 순수 표현이다. 위치·남은 시간은 <see cref="BombDevice"/>에서 읽고 상태를 바꾸지 않는다.
/// [임시] 아직 OnGUI로 그린다 — 캔버스 이전은 #493.
/// </summary>
