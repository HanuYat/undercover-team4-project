...
/// <see cref="NpcController.StartPenaltyChase"/>에 납치 임무 표식을 켜고, 범위를 벗어나도 같은 표적을
...
public partial class AbductionEvent : MonoBehaviour, ISuddenEvent
{
    ...
    public void ServerBegin()
    {
        ...
        for (int i = 0; i < m_abductors.Count; i++)
        {
            m_abductors[i].OnPenaltyCaught += HandleAbductionCaught;
            m_abductors[i].OnDamaged += HandleAbductorDamaged;
            m_abductors[i].StartPenaltyChase(target, abductionDuty: true);
        }
