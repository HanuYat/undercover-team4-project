...
/// "컴포넌트 1개 = 이벤트 1종"이 성립하지 않는 경우를 위한 통로다 — 인스펙터 리스트 항목마다 이벤트가
/// 하나씩 나오는 구성이 여기 해당한다.
///
/// <b>지금은 구현체가 없다</b> — 유일한 구현이던 <c>SpawnedNpcEventSet</c>이 이벤트별 컴포넌트로 갈렸다(#303).
/// 확장 지점으로 남겨 둔다: 매니저가 이 통로를 이미 타므로 다시 필요해지면 구현만 붙이면 된다.
///
