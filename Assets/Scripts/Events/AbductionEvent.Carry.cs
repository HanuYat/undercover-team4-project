...
public partial class AbductionEvent
{
    ...
    /// <summary>
    /// 끌고 가던(또는 아직 쫓는 중인) 표적이 <b>납치 밖의 사유로</b> 쓰러졌다 — 그러면 납치는 손을 뗀다. (#554/#679)
    ///
    /// <b>포획 전</b> — 추격 중인 표적이 죽으면(차 사고 등) 즉시 무산한다 (#679). 납치범은 포획해야
    /// 때릴 수 있으니(린치는 포획 후) 이 단계의 죽음은 항상 외부 사인이다. 표적이 그 자리에 쓰러져
    /// 있을 뿐 오브젝트가 파괴되진 않아 <see cref="NpcChaseState"/>가 스스로 눈치채지 못하므로,
    /// 기존 60초 상한을 기다리지 않게 여기서 앞당겨 끊는다.
    ///
    /// <b>포획 후</b> — 실제로 밟은 경로는 폭탄이다: 호송 중인 피해자가 폭발에 맞아 HP가 0이 되면
    /// <see cref="PlayerHealth"/>가 곧바로 기능 정지를 건다(#524). 접수 단계 가드(<see cref="HandleAbductionCaught"/>)만
    /// 으로는 <b>접수한 뒤</b> 원인이 바뀌는 경우를 못 잡아, 폭탄사에도 시체가 계속 끌려가 반출까지
    /// 이어지고 있었다. <b>린치 중</b>에는 납치 스스로도 사인을 바꾸므로(Lynched → Die) 마지막 가해자로
    /// (<see cref="m_lastVictimAttacker"/>) 갈라 낸다 — 납치범 주먹이면 결말을 그대로 두고, 외부 사인이면
    /// 물러난다. 반출은 부활 기회까지 지우므로 폭탄과 겹쳤다는 이유만으로 그걸 잃게 하지 않는다 (2026-08-07).
    ///
    /// 정적 이벤트라 <b>모든</b> 플레이어의 변경이 들어온다 — 지금 쫓거나 끌고 가는 대상만 본다.
    /// </summary>
