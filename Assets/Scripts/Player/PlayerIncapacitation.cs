...
public class PlayerIncapacitation : NetworkBehaviour
{
    ...
    [SerializeField] private GameObject m_reviveHitbox;
    ...
    public bool IsIncapacitated => IsSpawned && !IsServer ? m_isIncapacitatedSynced.Value : m_isIncapacitated;
    ...
    {
        if (IsSpawned && !IsServer) return; // 서버 권위 방어 (PlayerEscorter 관례)
    ...
    {
        if (m_isIncapacitated == value) return; // 중복 트리거 무시
