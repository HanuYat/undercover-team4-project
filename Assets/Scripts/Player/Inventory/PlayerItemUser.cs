...
public class PlayerItemUser : MonoBehaviour
{
    ...
    {
        // 커서가 풀려 있으면 좌클릭은 UI 것이다 — 인벤토리 편집(I)은 슬롯을 좌클릭으로 끌기 때문에
        ...
        // 다중 끌기의 대가는 슬롯 경쟁(밧줄로 5칸을 채우면 들 것이 없다)과 무게 페널티(#398)가 진다.
        // 동료 운반(#365)도 같은 밧줄이라 같은 규칙을 따른다 — 업고 가면서 다른 아이템을 쓸 수 있다.
    ...
    /// 좌클릭을 누른 채 커서가 풀리는 경로(인벤토리 편집 I)도 이걸 부른다. (#352)
    /// </summary>
