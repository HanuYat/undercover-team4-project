...
///
/// 이 영역이 <b>"유치장 안이냐"의 단일 기준</b>이다: 시민의 통행 차단(NpcController.SetJailAccess)과
/// 착석 판정(JailIntake의 R2)이 같은 폴리곤을 본다.
/// 차단은 문이 아니라 NavMesh 영역이 한다 — 통행이 없으면 문이 열려 있어도 경로가 잡히지 않는다.
///
/// <b>검거 판정은 이 영역을 보지 않는다</b> — 문 앞 게이트(<see cref="JailScanner"/>)가 쥔다 (#492).
/// 한때 Jail 영역 진입이 판정 트리거였는데, 유치장 안에서 확정된 오검거가 통행을 잃고 굳어서
/// 판정을 문 밖으로 뺐다. 둘을 헷갈리지 말 것: <b>이 영역은 '가둔 곳', 게이트는 '가를 곳'</b>이다.
///
