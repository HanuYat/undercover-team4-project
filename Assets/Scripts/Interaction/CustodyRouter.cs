...
/// <summary>
/// 신병 처리 — 검거 판정(ArrestJudge) 결과에 따라 NPC의 행선지를 정한다. (GDD 7-2, #228)
/// #492로 <b>오검거 갈래만 남은 얇은 라우터</b>가 됐다:
///
/// - 현상수배범·경범죄 → <b>여기서는 아무것도 하지 않는다</b>. 수용은 플레이어가 직접 끌고 들어가
///   좌석에 놓을 때 JailIntake가 배정·계상한다 (#492 — 자동 이송 폐기)
/// - 오검거(무고한 시민) → 수갑 해제 후 석방(배회 복귀)
///
/// 수감 갈래가 JailIntake로 옮겨져 이제 유치장(JailZone)을 알지 않는다 — 남은 일은 오검거 페널티
/// 매니저가 없는 씬에서의 석방 폴백뿐이라, 그 매니저가 상시 배치되면 이 클래스는 지워도 된다.
