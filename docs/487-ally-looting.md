...
| 닫기 | **ESC · E 둘 다** | 연 손이 그대로 닫는다. ESC만이던 시절엔 열 때 쓴 손가락이 다른 키로 옮겨 가야 했다 (§4-6) |
| 부활 후 | **안 돌려준다** | |
...
### 4-6. E로 닫기 — 폴링이어야 하는 이유

창은 열릴 때 스스로 게임플레이 입력을 정지시킨다(`PlayerInputHandler.SetSuspended`). 정지는 상호작용
액션까지 끄므로 **`OnInteractPerformed`가 오지 않는다** — 그래서 E 닫기를 이벤트로 받을 수 없고,
`LootPanel.Update`에서 `PlayerInputHandler.WasInteractPressedThisFrame()`으로 묻는다. 그 함수는 액션이
아니라 **바인딩된 컨트롤을 직접** 읽어 정지 중에도 답한다. (E를 상수로 박지 않은 이유는 조준 안내가
쓰는 `InteractBinding`과 갈라지지 않게 하려는 것이다 — #664)

**같은 누름으로 열고 닫히지 않게 프레임을 기록한다.** 호스트·오프라인에서는 E 콜백 안에서 서버
검증까지 한 프레임에 끝나(`HasServerAuthority` 직행) `Open`이 같은 프레임에 돌고, 입력이 `Update`보다
먼저 처리되므로 그 프레임의 `Update`에서도 E는 여전히 "이번 프레임에 눌림"이다. `m_openedFrame`이
그 한 프레임만 건너뛴다. 원격 클라는 RPC 왕복 뒤에 열려 애초에 겹치지 않는다.

**닫은 직후 E가 눌린 채여도 다시 열리지 않는다.** 상호작용 액션은 `Button`이고
`initialStateCheck: false`라(`InputSystem_Actions.inputactions`) 다시 켤 때 이미 눌린 키로 발동하지
않는다. 이 플래그가 켜지면 닫자마자 재개된 액션이 그 자리에서 창을 다시 열어 깜빡인다.
...
| ESC → 커서·이동 복귀 | ⚠ 미검증 |
| **E → 닫기** (같은 누름으로 열고 닫히지 않는지 · 닫은 뒤 재오픈 없는지) | ⚠ 미검증 |
| 슬롯 꽉 참 거부 | ⚠ 미검증 |
| 배터리 잔량 유지 | ⚠ 미검증 |
| 라운드 종료·상점 배달 | ⚠ 미검증 |
...
1. 6장 미검증 항목 — 남은 것은 **ESC 복귀 · E 닫기 · 슬롯 꽉 참 거부 · 배터리 잔량 · 상점 배달**.
