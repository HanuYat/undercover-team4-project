...
> **이후 변경 (2026-08-03):** 판정 트리거가 이 계획의 **Jail 영역 진입**에서 **유치장 문 앞 보안 스캐너 게이트**(`JailScanner`)로 바뀌었고, 통행 부여 기준도 '신병 여부'에서 '판정 통과 여부'로 바뀌었다. 설계 문서의 위험 항목이 예측한 대비책을 택한 결과다 — 근거는 그 문서 머리말과 GDD 7-2 #492 노트 참고. 아래 Task 본문은 실행 당시 기준이라 R1의 판정 기준 서술이 현행과 다르다.
