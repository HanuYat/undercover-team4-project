...
| 파일 | 책임 | 태스크 |
|---|---|---|
| `Assets/Scripts/NPC/Profile/CitizenIdentity.cs` | `SetCriminal(bool)` — 범인 여부만 떼어낸 진입점 | 1 |
| `Assets/Scripts/NPC/Appearance/AppearanceAssigner.cs` | 몽타주 발행을 공개분만으로 게이트 + `RevealMontage()` | 2 |
| `Assets/Scripts/NPC/Profile/CriminalAssigner.cs` | 예비 풀 확정 / 공개 분리 | 3 |
| `Assets/Scripts/NPC/Profile/CriminalAssigner.cs` | `PromoteNext()` · `HasPendingSuspect` + 승격 가드 | 4 |
| `Assets/Scripts/HQ/Tip/TipCallPhone.cs` | **신규** — 서버 수신 타이머 + `IInteractable` | 5 |
...
**Files:**
- Modify: `Assets/Scripts/NPC/Profile/CitizenIdentity.cs`
...
`Assets/Scripts/NPC/Profile/CitizenIdentity.cs`의 `AssignProfile` 메서드 **바로 아래**, `AssignAppearance` XML 주석 **바로 위**에 삽입한다:
...
```bash
git add Assets/Scripts/NPC/Profile/CitizenIdentity.cs && git commit -m "제보 전화 — 범인 여부만 여는 SetCriminal 진입점 (#102)"
```
...
**Files:**
- Modify: `Assets/Scripts/NPC/Appearance/AppearanceAssigner.cs`
...
```bash
git add Assets/Scripts/NPC/Appearance/AppearanceAssigner.cs && git commit -m "제보 전화 — 몽타주 발행을 공개분으로 게이트 + RevealMontage (#102)"
```
...
**Files:**
- Modify: `Assets/Scripts/NPC/Profile/CriminalAssigner.cs`
...
```bash
git add Assets/Scripts/NPC/Profile/CriminalAssigner.cs && git commit -m "제보 전화 — 예비 용의자 풀 확정과 공개 분리 (#102)"
```
...
```bash
git add Assets/Scripts/NPC/Profile/CriminalAssigner.cs && git commit -m "제보 전화 — PromoteNext와 승격 가드 (#102)"
```
...
**Files:**
- Create: `Assets/Scripts/HQ/Tip/TipCallPhone.cs`
...
```bash
git add Assets/Scripts/HQ/Tip/TipCallPhone.cs Assets/Scripts/HQ/Tip/TipCallPhone.cs.meta && git commit -m "제보 전화 — 본부 전화기 수신·응답 (#102)"
```
