...
**확정(결정 7):** `ResolveTarget()`이 null인 상태가 **5초** 이어지면 `Idle`로 복귀시킨다.

> **보강(리뷰 지적, 2026-07-27):** "표적 없음"의 정의가 빠져 있었다. `ThreatTarget`은 오브젝트가 파괴될 때만 null이 되므로, 그대로 두면 수갑으로 저항을 유발한 플레이어가 도망쳐도 표적이 계속 잡혀 타이머가 매 틱 리셋된다 — 제한시간 도주를 없앤 뒤로는 이게 `Attack`의 유일한 출구라 저항 NPC가 라운드 끝까지 따라붙는다. 진입점 4곳 중 3곳이 유발자를 넘기므로 주 경로 전부가 해당된다. → 유발자가 **`GiveUpDistance`(25m, 도주의 이탈 거리와 동일) 밖으로 벗어나거나 다운(`IsTargetable == false`)되면 표적 상실로 본다.** 다운 조건이 필요한 이유는 폴백 경로만 `IsTargetable`로 거르고 있어, 유발자 경로로 들어오면 쓰러진 플레이어를 영구히 쫓기 때문이다. 도주가 아니라 배회다 — 때릴 상대가 사라진 NPC가 혼자 전력 질주하지 않는다. (결정 5가 도주 복귀로 원복된 뒤에도 이 항목은 배회 그대로다. 기절 해제는 방금 얻어맞은 직후라 상황이 다르다.)
