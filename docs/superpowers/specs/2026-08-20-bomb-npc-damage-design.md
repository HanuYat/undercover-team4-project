...
- **이미 정착한 시체는 건드리지 않는다.** `NpcRagdoll.EnterRagdoll`이 `"이미 정착했다 — 다시 날리지
  않는다"`로 스스로 거부한다. 기존 결정이고, 뒤집으면 유치장까지 끌고 가야 판정이 나는 시체가 폭발마다
  흩어진다.
...
| 이미 정착한 시체의 재비행 | 일어나지 않는다 | `EnterRagdoll`이 거부 |
