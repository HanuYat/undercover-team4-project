...
- **이미 정착한 시체는 건드리지 않는다.** 유치장까지 끌고 가야 판정이 나는 시체의 자리를 지킨다.

  > ⚠ **정정(2026-08-20).** 이 문단은 원래 `NpcRagdoll.EnterRagdoll`이 `"이미 정착했다 — 다시
  > 날리지 않는다"`로 스스로 거부한다고 적었다. **사실이 아니다.** 정착한 시체는
  > `m_state == RagdollState.Ragdoll`이므로 첫 분기에 걸려 `ApplyImpulse`까지 **들어간다**
  > (`RagdollState`는 `Animated`/`Ragdoll` 둘뿐이고 정착은 `m_settled` bool이 든다 — 세 번째
  > 상태 `Settled`가 있는 것은 `PlayerRagdoll`이다). 그래서 `wasAlive` 검사가 유일한 방어다.
  >
  > **다만 실패 모드는 "흩어진다"가 아니다 (2026-08-20 Play 모드 실측).** 정착한 시체의 뼈
  > Rigidbody 12개는 전부 `isKinematic == true`라 속도 대입이 무시된다 — 실제로 일어나는 일은
  > **뼈마다 Unity 경고 1회(총 11~12회)와 몸이 물리로 깨어나며 약 0.4m 밀리는 것**이다
  > (kinematic 12→1). 회수 위치가 어긋날 만큼은 아니지만 콘솔이 도배되고, 무엇보다
  > `RagdollRig.ApplyImpulse`가 그 파일에서 유일하게 `isKinematic` 가드가 없는 루프라는 사실이
  > 드러난다(형제 `ClampSpeed` 등 4개는 전부 거른다). 그쪽 처리는 이 이슈 밖이다.
