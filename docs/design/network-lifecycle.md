...
위 ③은 **"로그아웃" 시나리오에만** 포함된다. "메인으로 나가기"(`SessionFlow.LeaveToMainAsync`)는 ①②까지만 수행하고 **인증은 유지한다** — 타이틀로 돌아온 직후 로그인 절차 없이 바로 방을 만들거나 참가할 수 있어야 하기 때문이다.

- 원칙 4의 게이트는 그대로 유효하다. 세션을 이미 비운 뒤라 이후 **어느 시점에** 로그아웃해도 순서 위반이 구조적으로 불가능하다.
- 명시적 로그아웃은 별도 경로가 전담한다 — 타이틀 씬의 `AuthPanel` 로그아웃 버튼이 `Auth.SignOut()`을 직접 부른다. 그 패널은 타이틀에만 배선돼 있고, 세션 중이면 `IsNetworkConnected` · `CanSignOut` 가드가 다시 막는다.
