...
| **세션 상주 홀더** → `App.Game.TeamFund` · `App.Game.ShopPurchases` · `App.Game.RoundProgress` 등재 | `SessionObjectSpawner`가 **Title 씬에서 런타임 스폰**해 씬을 넘어 사는 오브젝트들(`TeamFund.prefab` 하나에 셋이 얹혀 있다)이라, 게임 씬의 인스펙터로 배선할 방법이 아예 없다 — SceneReadyGate·HUD 항목과 같은 사정이다. 셋 다 도메인 밖 코드 참조가 Round 하나뿐이라 **R3 ②에는 미달**한다(`TeamFund`에 대한 HQ·Interaction의 언급은 전부 주석이다 — 참조를 셀 때 §1의 (a)에 걸린다). 세션을 넘어 사는 상태를 다루는 홀더가 늘어나면 이 항목에 함께 적는다. 참조 도메인이 둘 이상으로 퍼지면 예외가 아니라 정규 등록이 된다 (#214·#377) |
