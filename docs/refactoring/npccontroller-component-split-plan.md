...
**Play 회귀는 아직 돌리지 않았다** — § 9-4의 위험 세 가지(프리팹 배선 누락 · `NetworkVariable` 인덱스 ·
쓰기 방향이 바뀐 경로)에 걸리는 항목을 골라 MPPM 2인으로 확인할 것: 밧줄 끌기·놓기·줄다리기 /
줄 풀림 기상과 재포획 창(#513) / 폭발 넉백 / 테이저 스턴 + 밧줄 콤보(#390) / **원격 클라에서 누운 자세**
(`m_ropedSynced`·`m_tetheredSynced`·`m_standUpPendingSynced` 셋이 전부 클라 표현용이라, 인덱스가
어긋나면 여기서 먼저 티가 난다).
...
- [ ] **Play 회귀 (MPPM 2인)** — 7번은 아직 안 돌렸다. 항목은 § 6 "7번 결과" 마지막 문단
