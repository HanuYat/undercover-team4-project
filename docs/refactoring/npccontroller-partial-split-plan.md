...
> **[대체됨 — 2026-08-05]** 이 문서의 partial 분리는 #259로 **완료됐다**(2026-08-06 기준 12파일 1,978줄). 다음 단계인
> 부품 컴포넌트 분리 계획은 [npccontroller-component-split-plan.md](npccontroller-component-split-plan.md)(#503)에 있고,
> 아래 § 1의 "컴포넌트 분리 비추천" 판단은 그 문서 § 2의 실측으로 **뒤집혔다** — 경로 변경은 상태 클래스 76건 +
> partial 간 16건이다. (수치는 새 문서가 정본이다 — 여기 적힌 값은 그 시점의 사본일 뿐이니 어긋나면 새 문서를 믿을 것)
> 이 문서는 partial 분리의 기록으로 남긴다. **§ 5 검증 절차는 새 계획에서도 계속 쓴다.**
