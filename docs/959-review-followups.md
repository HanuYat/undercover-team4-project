...
### 2-1. 권위 축은 열어 둔다 — 알고 남긴 것이다 (2026-09-02 PR #959 리뷰)

같은 함수에 축이 하나 더 있다. **권위를 잃은 피어에서도 `SetKinematic(false)`가 걸린다** — 자세를 스트림으로 받는 피어는 전 뼈가 키네마틱이어야 한다는 `ReleaseBonesToPhysics`의 불변식을 어기는 것이고, `TickAuthorityHandover`가 `m_hadMoveAuthority` **엣지 트리거**라 스스로 못 돌아온다(이관 프레임에 `FixedUpdate`가 0회 돌면 `Update`가 엣지를 먼저 먹고, 다음 프레임 `FixedUpdate`가 뼈를 열어 둔 채 굳힌다).

**그런데 도달 경로가 하나뿐이고 그 경로가 증상을 스스로 감춘다.** `Beamed`는 소유권을 옮기지 않는 사유라, 권위를 잃으며 래치가 살아 있는 조합은 **확정 흡입**(`Swallow → ServerKillByBodyLost`)이 유일하다 — 탈출 셋(라운드 종료·`OnDisable`·흡입 타임아웃)과 빔 중 타 사유 사망(`StopTow`, 이관이 미뤄져 권위 유지)은 전부 권위를 쥔 채 래치를 푼다. 그리고 `Swallow`는 같은 호출에서 `SetBodyLost(true)`로 렌더러를 끄고, 서버가 권위를 잡아 자세 스트림이 매 패킷 덮어쓰며, `IsBodyLost`라 부활 경로도 없다.

**그래서 가드를 넣지 않았다** — 관측된 증상이 없는 결함에 방어 조건을 다는 것이고, `PlayerRagdoll`은 이미 실코드 1,132줄이다. **되살아나는 조건은 하나다: 흡입 결말이 "몸을 남기는" 쪽으로 바뀌는 것**(GDD 7-5는 아직 초안 절이다). 그때는 `m_state` 가드 옆에 `|| !HasMoveAuthority`를 더하면 된다 — 위 탈출 셋은 이 조건에 안 걸리므로 회귀 위험이 없다.
