...
> **결말: 채택됐다 — §7, 그리고 재사망 버그 셋은 §8에서 해결.**
> §5가 "Hips 동기화는 실패"라고 적고 있지만 **그 결론은 뒤집혔다.** 빠져 있던 조각은 밧줄을
> **권위 피어만 묶는 것**이었고, 이후 §5의 침하가 한 번 재현됐을 때도 원인은 A안이 아니라
> **뼈 길이 누적**(§8-2)이었다. §5까지 읽고 이 방향을 닫지 말 것.
>
> 남은 미해결: **부활 시 큰 회전**(§4), **§8-3의 A안 무죄 확정**(같은 자리에서 2·3차 사망 재현).
...
- **§5의 −2.509m를 재현하지 못했다.** 구성상 확실히 달라진 것은 밧줄이 권위 전용이 된 것 하나인데,
  ...
  > **§8에서 재현됐고, 원인이 A와 무관한 것으로 드러났다.** 이 문단은 그대로 두되 §8을 먼저 읽을 것.
...
## 8. 재사망이 깨진다 — 서로 다른 버그 셋 (해결)

§7 직후 "죽는 순간 시체가 T자에서 출발한다"를 파다가, **겹쳐 있던 별개의 버그 셋**이 차례로 나왔다.
셋 다 A안(골반 복제)과 무관하고, 셋 다 진단값이 원인을 직접 가리킨 뒤에야 잡혔다.

### 8-1. T자에서 시작 — 포즈가 PhysX에 안 갔다

| 진단 | 값 |
|---|---|
| `복사잔차` (살아있는 리그 ↔ 시체 리그 최대 회전차) | **0.0°** — 복사는 트랜스폼에 완벽히 닿았다 |
| `물리반영차` (Rigidbody 액터 ↔ 트랜스폼) | **81.7°** — 액터만 옛 포즈를 들고 있었다 |

이 프로젝트는 `m_AutoSyncTransforms: 0`이라(`ProjectSettings/DynamicsManager.asset`) 트랜스폼에 쓴
값이 PhysX로 **즉시 넘어가지 않는다.** 키네마틱인 동안은 트랜스폼이 진실이지만 동적으로 바뀌는
순간 **액터가 진실**이 되므로, 그 사이에 동기화하지 않으면 물리가 액터의 바인드 포즈에서 출발한다.

**고침:** `RagdollRig.SetKinematic(false)` 앞에 `Physics.SyncTransforms()`.
호출부가 아니라 리그에 넣었다 — 같은 전이가 네 군데다(사망 시 시체 켜기, 정착 후 물리 복귀,
NPC 래그돌 진입, 얼린 시체 녹이기). 하나 빠뜨리면 조용히 돌아온다.

### 8-2. 2차 사망에 사지가 늘어나며 바닥을 뚫는다 — **뼈 길이가 누적된다**

증상: 부활 후 다시 죽이면 관절이 고무처럼 늘어나며 발이 바닥을 지나 허공에 매달린다.

먼저 **틀린 가설 둘을 실측으로 버렸다:**

| 가설 | 반증 |
|---|---|
| 부활 후 살아있는 리그가 누운 채로 남는다 (§4 애니메이터 컬링) | 2차 사망 시 `살아있는기울기=4°`, `손간격=0.72` — 정상 |
| 뼈 콜라이더가 꺼져 있다 / 캡슐이 켜져 있다 | `뼈콜라이더=11/11`, `캡슐=꺼짐` |

진짜 원인은 **회전만 보던 진단이 놓치던 축**이었다:

```
뼈길이드리프트 (관절 달린 뼈의 로컬 위치 ↔ 바인드 포즈)
  1차 사망  0.0055m
  2차 사망  0.0624m   ← 11배, 사망마다 누적
```

관절의 `connectedAnchor`는 **바인드 포즈 기준으로 구워진다**(프리팹이
`AutoConfigureConnectedAnchor: 1`). 골격이 6cm 어긋난 채 물리에 넘어가면 **첫 스텝부터 관절이
위반된 채 출발한다.**
...
```
1차 사망   물리가 관절을 늘린다
정착       RestoreCapturedPose가 월드 포즈를 쓴다 → 늘어난 길이가 localPosition에 굳는다
부활       CopyPose(시체→살아있는) 가 그 길이를 살아있는 리그로 옮긴다
살아있는 동안  애니메이터는 회전만 쓴다 → 길이가 그대로 남는다
2차 사망   CopyPose(살아있는→시체) 가 도로 주입 + 시체 자신의 잔여분과 합쳐진다
```

**고침 — 두 조치가 짝이다. 하나만으로는 다른 경로로 되돌아온다:**

| | 막는 것 |
|---|---|
| `RagdollPose.Copy`가 **관절 달린 뼈의 `localPosition`을 안 옮긴다** | 리그 사이 전파 |
| 부활 시 `RagdollRig.RestoreBindPose()` | 시체 자신의 누적 |

⚠ `Copy`는 **양쪽 다** 검사해야 한다(`from`·`to`). 관절은 시체 리그에만 있고 복사 방향은 둘이라,
받는 쪽만 보면 부활 방향(시체→살아있는)이 그냥 통과해 늘어난 길이가 세탁된다.

**결과:** 3회 사망 × 3피어 전부 `뼈길이드리프트=0.0000`, 바닥 박힘 없음.

> **관절 달린 뼈만 세야 한다.** 처음엔 리그 전체를 쟀는데 거기 **골반**이 들어 있다 — 골반은
> 래그돌 루트라 로컬 위치가 자세의 일부이고 애니메이션에 따라 정당하게 변한다. 그래서 아무 문제가
> 없는데도 2cm대가 상시로 찍혀 경고가 무의미해졌다.

### 8-3. §5의 A안 실패는 이것이었을 가능성이 높다

§7이 "재현 못 했다"고 적은 침하가 **8-2를 파는 도중 재현됐다** — 그리고 관측된 것은 전부
**2차 사망**이었다:

```
골반Y=0.918  골반-지면=0.617        골반은 스트림에 고정 (키네마틱)
최저뼈-지면=-0.694                   발은 지면 0.69m 아래
골반↔최저뼈=1.390 ⚠관절위반          골격 한계 0.9 초과
골반밑=..._Wood_Floor_Raised_x2_01@0.301   발밑=없음   ← 띄워 올린 바닥을 지나 허공
```

8-2를 고친 뒤 같은 조작에서 **박히지 않는다.** 즉 §5가 "골반 키네마틱의 구조적 한계"로 읽은 것이
실은 **뼈 길이 오염**이었을 수 있다.

⚠ **아직 확정 아니다.** 확정하려면 **같은 자리(띄워 올린 나무 바닥)에서 2·3차 사망**을 다시 밟아야
한다. 재현되지 않으면 A안은 무죄이고, §6의 B·C안은 필요 없다.

### 이 아크의 교훈

- **1초 간격 스냅샷으로는 물리 스텝(50Hz) 현상이 안 보인다.** 매 스텝 쌓아 요약해야 톱니가 보인다.
- **대조군 없이는 어떤 증상이 내 변경 때문인지 알 수 없다.** 실험 스위치에 "끄면 main과 같은 동작"
  자리를 반드시 둘 것.
- **회전만 재면 위치 버그를 못 본다.** `복사잔차=0.0°`가 세 번이나 "복사는 정상"이라고 말하는 동안
  뼈 길이가 11배로 늘고 있었다.
- 추론으로 고치기 전에 **가설을 가르는 값**을 먼저 넣을 것. 이 아크에서 추론으로 고친 시도는
  전부 틀렸고, 진단값을 넣은 뒤에는 한 번에 잡혔다.
