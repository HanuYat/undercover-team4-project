...
> **상태: 커밋 ①②③ 구현 완료, 커밋 ③의 임펄스 경로는 플레이 검증 대기.**
> 2026-08-05 계획 작성 → 커밋 ①②③ 착수, 2026-08-06 캡슐 추종을 스윕+텔레포트로 정리.
> "시체가 마지막에 몇 번 튄다"는 **캡슐 추종 설계(§3-4)로 해결됐다** — 그 뒤에 딸려 나온 위치 동기화
> 문제들(§9-9~§9-12)까지 원인과 처방을 **§9**에 전부 남겼다.
>
> **다음 세션에서 할 일:**
> 1. 폭발 사망을 오너·원격 **양쪽에서** 보고 §9-13의 판별점 확인 (한쪽만 보면 정상으로 보이는 버그를 두 번 겪었다)
> 2. 시체가 벽을 뚫지 않는지 (§3-4의 "뼈를 옮기는 경로" 표가 그 보장이다)
> 3. `캡슐 잔차 텔레포트` 로그 빈도 — 매번 나오면 임펄스가 지형에 비해 과하다
>
> 미해결: 밧줄 드래그 중 몸이 찢어지는 별건(§9-7, 커밋 ④), 오너 시점 회전(§3-5, 커밋 ④).
...
**원격은 착지한 뒤 뼈를 스트리밍된 루트로 당겨온다** (`TickAlignBonesToRoot`). 오너 캡슐이 골반을 따라오므로 **스트리밍된 루트의 수평 위치가 곧 오너 골반의 수평 위치**다 — 뼈를 따로 동기화하지 않고도 오너의 궤적을 받는다(결정 4를 지킨다). 리그 루트 오프셋으로는 못 고친다: **동적 리지드바디는 부모 트랜스폼을 따르지 않는다.** 그래서 뼈마다 같은 델타를 더해 강체로 평행이동한다 — 포즈·상대속도·관절이 보존되고, 동적 바디의 `position` 대입은 텔레포트라 속도가 유도되지 않는다(키네마틱과 반대 — §9-5).

> ⚠ **비행 중에는 당기지 않는다. 이걸 매 프레임 보정으로 짰다가 §9-10에서 되돌렸다.** 원격의 루트는 접지된 CharacterController이고 시체는 포물체다 — 둘은 매 프레임 크게 갈라지는 것이 정상이라, 차이를 전부 되돌리면 **시체의 궤적이 캡슐의 궤적으로 덮인다**. 정렬이 필요한 것은 최종 정착 위치뿐이고, 비행 중 피어 간 차이는 #506이 감수하기로 한 항목이다.
>
> 비행 중 보정을 안 하는 두 번째 이유는 **벽 통과**다 — 아래 참고.
...
| 착지 후 당겨오기 (`SnapBonesBy`) | **`ClampByWall` 라인캐스트 가드** — 벽에 막히는 지점까지만 옮긴다 |
| 정착 시 포즈 복원 (`RestoreCapturedWorldPoses`) | 직전에 물리로 점유했던 포즈를 되돌리는 것뿐 — 새 위치로 옮기지 않는다 |

비행 중에 뼈를 옮기는 경로는 **하나도 없다.** 거리 클램프(시체가 루트에서 N m 벗어나면 회수)를 한때 안전망으로 뒀다가 지운 이유가 이것이다 — 그게 유일하게 충돌을 무시하는 경로가 됐고, 게다가 상한을 정상 비행 거리보다 작게 잡아 매 폭발마다 발동했다(§9-11).
...
- `PlayerRagdoll` — `ReportResidualTeleport` / `m_reportedFollowStall`, 정착 로그의 `시체 이동`·`루트 이동`과 `m_entryRootPosition`
...
>
> **MPPM 가상 플레이어의 콘솔은 MCP 대상이 아니다** — 별도 프로세스라 메인 Editor의 콘솔만 읽힌다. 원격 쪽 로그가 필요하면 그 창에서 직접 복사해야 한다. 그래서 **검증하려는 쪽을 메인 Editor에 두는 것**이 중요하다(§9-13의 오너 경로가 오래 미검증이던 이유). §9-10·§9-11은 반대로 오너·원격을 <b>둘 다</b> 봐야 갈렸다 — 한쪽만 보면 정상으로 보인다.
>
> 그래서 **진단 로그에는 판정에 쓴 값을 함께 찍는다.** §9-12는 로그가 상한값을 같이 찍고 있었던 것만이 단서였다. "왜 안 고쳐지지"를 반복하게 만드는 종류의 함정은 값을 안 찍으면 보이지 않는다.
...
데미지 오버라이드는 빠졌다 — §4의 거리 감쇠가 들어간 뒤로는 폭심 150 / 가장자리 0.2로 즉사 반경 3.3m가 나오므로 폭심 근처에서 확실히 죽는다.

> **없애는 방법은 드롭이 아니라 `git revert`다** — PR 직전에 `git revert <리그 커밋>` 하나로 끝낸다.
>
> 드롭(히스토리 재작성)을 쓰지 않는 이유: 드롭이 깨끗하려면 리그 커밋을 항상 브랜치 최상단에 유지해야 하고, 그러면 **커밋을 추가할 때마다 재작성 + force push**가 된다. PC를 옮겨 가며 작업하면 다른 클론이 옛 tip을 들고 있는 사이에 force push가 나가 작업을 잃는 경로가 열린다. revert는 재작성이 없어 **평소처럼 커밋·푸시**하면 되고, 리그 커밋을 최상단에 둘 필요도 없다.
>
> (리그 커밋 메시지에는 "최상단에 유지할 것"이라고 적혀 있다 — 그건 이 방침으로 대체됐다. 메시지를 고치는 것 자체가 재작성이라 그대로 둔다.)
