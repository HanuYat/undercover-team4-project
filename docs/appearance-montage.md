...
1. **관통 시트** — 비교 축 `HairStyle`, 후보 프롭에 머리 39종(Generic 미사용 8 + PS 9 + Apoc 22),
   **같이 붙일 프롭 = `SM_Gen_Chr_Attach_Beanie_01`**. 비니를 검정 가림막으로 붙여 **밖으로 나온 머리만**
   그린다. 한 번에 두 답을 준다 — 다른 팩 머리가 쓸 만한가(어휘), 어느 것이 모자를 뚫는가(`HidesHair`).
2. **수염 시트** — 축 `FacialHair`, 후보 12종(Generic `Beard_01` + PS `Beard_01`·`Moustache_01/02` + Apoc 수염 8).
3. **SciFi 아틀라스 시트** — SciFi 섹션의 `아틀라스`에 `PolygonScifi_01_A~F`와 `_01_A/_02_A/_03_A/_04_A`를
   넣고 굽는다. 문자가 피부톤을 흔들면 SkinColor 축과 충돌하므로 숫자만 쓴다(§6-2).
4. 시트 결과로 값/미채택 분류 → 어휘 확장(새 옵션은 **배열 끝에 추가**할 것. 중간에 끼우면 인덱스가
   밀려 `AppearanceModelCatalog`가 통째로 어긋난다).
5. `HidesHair` → 축별 '없음 확률' → 마스크 축 순서. **'없음 확률'을 어휘 확장보다 뒤로 미루지 말 것** —
