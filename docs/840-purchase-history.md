...
2. **본부 게시판에 남은 소모품** — 상점에서 사서 아직 안 쓴 힐팩·부활 키트가 몇 개 남았는지. 기본 지급품·주운 물건은 세지 않는다(아래 6장).
...
- **설치형 설치 위치 표시** — 별개 관심사.
- **상점을 거치지 않은 물건은 세지 않는다** — 원본이 구매 집계라 기본 지급품·주운 물건·루팅분은 `ShopDeliveredItem` 표식이 없어 `RemoveCarried` 경로를 타지 않는다(`ItemBase.ServerConsume`). 게시판이 뜻하는 것은 팀 전체 보유량이 아니라 **구매 재고**다. 전체 보유량을 세려면 인벤토리를 순회하는 별개의 집계가 필요하다 — 이 이슈의 범위 밖이다.
