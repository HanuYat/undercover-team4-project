...
- 라운드는 수사 → 제압 → 연행 → 인계 → 판정을 반복합니다. 본부 CCTV와 현장 스캐너로 수배 명단을 대조하고, 제압한 대상을 밧줄로 묶어 감옥까지 끌고 갑니다

|  |  |
| :---: | :---: |
| ![현장 수사](docs/gifs/investigation.gif) | ![밧줄 연행](docs/gifs/escort.gif) |
| 팀 단위의 현장 수색 | 밧줄로 묶은 뒤 연행 |
...
전체적인 매니저 접근은 `App` 파사드 한 경로로 통일했고, 베이스 클래스를 상속하면 리플렉션으로 자동 등록됩니다.
...
| 검거 · 연행 | 팀원이 아이템으로 만든 밧줄을 기본 검거 수단으로 격상 — 다중 연행 · 무게 페널티 · 장력 반경 · 연결 동기화 | 설계 · 구현 |
| 팀 경제 루프 | 팀 공용 자금 · 구매품 이월 · 맵 선택 홀더, 라운드 정산 데이터, 상점 · Shop 씬 | 설계 · 구현 (일부 협업) |
| 에디터 툴 | MapGridBuilder · MeshSlicer · FacadeRunner · FPArmGenerator · ItemIconBaker, 맵 2종 제작 | 설계 · 구현 전담 |
| 튜토리얼 · UI | TutorialDirector, 인벤토리 핫바 · 스캔 HUD · ESC 패널, 커서 잠금 단일화 | 설계 · 구현 전담 |
...
- **구현** — 아이템 11종이 공유하는 사용 · 소지 · 네트워크 계약 설계
- **배경** — 아이템마다 사용 로직이 제각각이라, 새 아이템 하나를 추가할 때마다 플레이어 코드를 수정해야 했습니다
...
- **결과** — 아이템 11종 확장, 이후 팀원이 추가한 아이템도 `ItemBase` 상속만으로 동작. 반복되던 CTS 소유 · 재진입 가드를 ServerChannel로 추출, 팀원이 진압봉 임팩트 대기에도 재사용
...
- **구현** — 아이템이던 밧줄을 기본 검거 수단으로 격상, 여러 명이 한 대상을 함께 끄는 연행으로 확장
- **배경** — 기본 검거 수단이던 수갑은 소모품이라 회수 규칙이 복잡했고, 혼자서도 끝나 협동이 나오지 않았습니다. 팀 테스트와 회의를 거쳐, 팀원이 아이템으로 만들어 둔 밧줄을 기본 검거 수단으로 올리고 다중 연행 · 무게 · 장력을 얹었습니다
...
- **무게 페널티** — 끌고 있는 무게 = 대상 무게 ÷ 함께 끄는 사람 수, 속도 배율 = `clamp(1 − 0.25 × 무게, 0.35, 1)`. 나눠 끌수록 개인 감속이 줄어 협동이 이득이 되도록 했습니다
- **계산 분리** — 무게와 장력은 `RopeDragLoad`로 분리. 연결 목록은 표시 · 검증 · 인계가 함께 읽지만 이 계산은 `PlayerMovement`만 쓰기 때문입니다
...
- **구현** — 라운드를 넘어 이어지는 팀 자금 · 정산 · 상점 흐름
- **배경** — 검거 보상이 개인 단위로 쌓여 협동할 이유가 없었고, 라운드가 끝나면 자금 상태가 씬과 함께 사라져 다음 라운드로 넘길 방법이 없었습니다
...
- **구현** — 맵 제작 자동화 도구 5종, 전부 단독 작성
- **배경** — 격자 바닥 · 경계벽 배치가 맵 하나당 수백 개였고 맵을 추가할 때마다 반복됐습니다
- **MapGridBuilder** — ASCII 레이아웃 한 장 + `MapPalette`로 바닥 · 경계벽 생성. 건물은 풋프린트와 회전이 제각각이라 자동화에서 의도적으로 제외했고, 새 테마는 팔레트만 추가하면 됩니다
- **MeshSlicer** — 통째로 된 건물 모델에서 원하는 구간만 상자로 오려 벽 조각으로 저장합니다. 경계에 걸친 면은 그 자리에서 잘라 이어 붙이고, 잘린 자리의 무늬와 음영이 어긋나지 않게 맞춰 줍니다
...
#### 튜토리얼
- **TutorialDirector** — 로컬 호스트를 띄우고 안내 문구를 단계별 교체
- **단계 판정** — 안내 문구와 완료 조건을 한 배열에 나란히 적어 두고, 매 프레임 지금 단계의 조건 하나만 확인합니다. 통과하면 다음 단계로 넘어갑니다
- **기존 이벤트만 구독** — 튜토리얼을 붙이려고 게임 시스템에 이벤트를 새로 만들지 않았습니다. 장착 변경 · CCTV 전환 · 스캔 완료 · 검거 판정은 이미 있는 이벤트를 받아 완료 여부만 남기고, 이동 거리나 조준 대상처럼 이벤트가 없는 것은 매 프레임 값을 확인합니다
- **결과** — 매 프레임 도는 조건은 하나뿐이고, 게임 코드에는 튜토리얼 흔적이 남지 않습니다

#### 인게임 UI
- **구성** — 인벤토리 핫바, 스캐너 배터리 · 스캔 상태, 상호작용 라벨, ESC 일시정지 패널(`EscMenuGuard`)
- **커서 잠금** — 커서를 실제로 조작하는 곳은 `CursorLock` 하나입니다. UI는 해제를 요청하고 거두기만 합니다
- **커서 해제 요청 카운트** — 예전에는 UI가 열릴 때 커서 상태를 저장해 뒀다가 닫을 때 되돌렸습니다. 저장 자리가 전역에 하나뿐이라, 겹쳐 열린 UI가 연 순서와 다르게 닫히면(디스폰으로 모달이 저절로 닫히는 경우 등) 엉뚱한 값이 복원됐습니다. 지금은 해제를 요청한 UI 수만 세기 때문에 닫히는 순서를 신경 쓸 필요가 없습니다
- **씬 전환 대비** — 플레이어가 없는 동안(타이틀 · 로비, 라운드 종료 후)은 항상 해제해 둡니다. 잠긴 채로 씬을 넘어가면 풀어 줄 주체가 없어 로비 화면을 클릭할 수 없습니다
