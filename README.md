...
- **결과** — 아이템 11종 확장, 이후 팀원이 추가한 아이템도 `ItemBase` 상속만으로 동작. 반복되던 CTS 소유 · 재진입 가드를 `ServerChannel`로 추출해 사용처가 3곳 → 6곳
...
- **구현** — 여러 명이 한 대상을 함께 끄는 연행 시스템 (연결 동기화 · 무게 페널티 · 장력 · 배치)
- **배경** — 수갑 검거는 회수 규칙이 복잡했고 협동이 발생하지 않아, 팀 회의를 거쳐 기본 검거 수단을 수갑에서 밧줄로 변경했습니다
...
[v1.0.0 다운로드](../../releases/tag/1.0.0) — Windows 64bit
