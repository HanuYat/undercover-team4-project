...
- **구현** — 아이템 10종이 공유하는 사용 · 소지 · 네트워크 계약 설계
...
- **구현** — 개발 빌드에서 매 프레임 지표를 CSV로 남기는 계측기 제작
- **배경** — 빌드의 Profiler는 에디터로 스트리밍될 뿐 파일로 남지 않고, 캡처를 저장해도 `.raw` 바이너리라 수치화가 어렵습니다
- **동작 방식** — `ProfilerRecorder` · `ProfilerMarker`로 매 프레임 읽어 CSV 기록. `RuntimeInitializeOnLoadMethod`로 자체 생성해 씬 diff가 없고, `DEVELOPMENT_BUILD` 조건부 컴파일로 릴리스에서 제외됩니다
